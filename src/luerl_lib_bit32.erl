...
flshift(As, St) ->
    case luerl_lib:tointegers(As) of
    [X,Y|_] -> {[(checkint32(X) bsl trunc(Y))], St};
    _ -> badarg_error('lshift', As, St)
    end.

frshift(As, St) ->
    case luerl_lib:tointegers(As) of
    [X,Y|_] -> {[(checkint32(X) bsr trunc(Y))], St};
...
farshift(As, St) ->
    ...
    [X,Y|_] ->
        Disp = trunc(Y),
        case Disp > 0 of
            true -> {[(checkint32(X) bsr trunc(Y))], St};
            false -> {[(checkint32(X) bsl abs(trunc(Y)))], St}
        end;
...
flrotate(As, St) ->
    case luerl_lib:tointegers(As) of
    [X,Y|_] -> {[(lrotate(checkint32(X), trunc(Y)))], St};
    _ -> badarg_error('lrotate', As, St)
    end.

frrotate(As, St) ->
    case luerl_lib:tointegers(As) of
    [X,Y|_] -> {[(rrotate(checkint32(X), trunc(Y)))], St};
...
fextract(As, St) ->
    case luerl_lib:tointegers(As) of
    [N,Field,Width|_] ->
        {[(extract(N, Field, Width, As, St))], St};
    [N,Field|_] ->
        {[(extract(N, Field, 1, As, St))], St};
...
freplace(As, St) ->
    case luerl_lib:tointegers(As) of
    [N,V,Field,Width|_] ->
        {[(replace(N, V, Field, Width, As, St))], St};
    [N,V,Field|_] ->
        {[(replace(N, V, Field, 1, As, St))], St};
